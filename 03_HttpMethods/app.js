...
app.put("/item/:name", function(request, response) {
  ...
  /* Is the item in collection? */
  if (items.includes(itemName) && newName != null) {
    items[items.indexOf(itemName)] = newName;
    response.send(`<p>Item "${newName}" replaced "${itemName}" in collection successfully!</p>`);
  } else {

    response.send(`<p>Item "${itemName}" doesn't exist in collection or you don't give a new item name</p>`);
  }
});
